...
public class EnemyAnimation : MonoBehaviour {
	...
	void Update () 
	{
		if(target != null)
		{
        	...
        	nav.SetDestination(target.transform.position); // Set the destination of the NavMeshAgent to the position of the target
	...
	void DetermineAnimParameters (out float speed, out float angularSpeed)
	{
		...
		speed = 0;
		Vector3 projectedVelocity = Vector3.Project(nav.desiredVelocity, transform.forward);
	...
	float FindAngle (Vector3 fromVector, Vector3 toVector, Vector3 upVector)
	{
		...
		return Mathf.Atan2(Vector3.Dot(upVector, Vector3.Cross(fromVector, toVector)),Vector3.Dot(fromVector, toVector));
	}
	
	public void setTarget(Transform target, float speed = 2.0f, float stopDist = 0)
	{
		this.target = target;
		nav.speed = speed;
	}
