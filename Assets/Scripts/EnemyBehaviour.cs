...
public class EnemyBehaviour : MonoBehaviour {
	...
	public bool PlayerInSight(GameObject player, GameObject npc)
	{
		...
		if (angle <= fieldOfViewAngle * 0.5f)
		{
			...
			if (Physics.Raycast(npc.transform.position + npc.transform.up, toPlayer.normalized, out hit, sightRange))
			{
...
public class PatrolState : FSMState
{
	...
	public override void Reason(GameObject player, GameObject npc)
	{
		...
		if (npc.GetComponent<EnemyBehaviour>().PlayerInSight (player, npc)) {
			...
			// base.AddTransition(Transition.SawPlayer, new ChasePlayerState(npc, player));
...
public class ChasePlayerState : FSMState
{
	...
	public override void Act(GameObject player, GameObject npc)
	{
		...
		enemyAnimation.setTarget(player.transform, chaseSpeed, stopDist);
	}	
} 
